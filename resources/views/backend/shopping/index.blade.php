        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-content">
                    <div class="card-body">
                        <h4>Ürünler </h4>
                        <a href="{{   route('admin.shopping.create')   }}" class="btn btn-primary mb-2 float-right"><i
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td align="left"> <img style="height: 50px;width: 50px;" class="img-circle"
                                                src="{{  URL::to('uploads/shopping/')}}/{{ $shop->image }}"></td>
                                        <td>{{ $shop->name }}</td>
                                        <td>{{ $shop->description }}</td>
                                        <td>{{ $shop->type }}</td>
                                        <td>{{ $shop->price }}</td>
                                        <td>
                                            <a href="{{   route('admin.shopping.edit',$shop->id) }}"> <i
                                                    class="feather icon-edit font-medium-5"></i> </a>
                                            <a href=""
                                                onclick="if(confirm('Are You sure you want to delete this')){event.preventDefault();document.getElementById('delete-form-{{ $shop->id }}').submit();}else{event.preventDefault();}">
                                                <i class="feather icon-trash  font-medium-5"> </i></a>
                                            <form id="delete-form-{{ $shop->id }}" method="post"
                                                action="{{ route('admin.shopping.destroy',$shop->id) }}">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                            </form>
